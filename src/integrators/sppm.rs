            ...
            for iteration in pbr::PbIter::new(0..self.n_iterations) {
                                            ...
                                            for depth in 0..integrator.max_depth {
                                                ...
                                                if scene.intersect(&mut ray, &mut isect) {
                                                        ...
                                                        isect.get_bsdf(&arena_bsdf)
                                                    {
                                                        ...
                                                        let is_diffuse: bool = bsdf.num_components(
                                                            bsdf_flags,
                                                            &arena_bxdf,
                                                        ) > 0;
                                                        bsdf_flags = BxdfType::BsdfGlossy as u8
                                                            | BxdfType::BsdfReflection as u8
                                                            | BxdfType::BsdfTransmission as u8;
                                                        let is_glossy: bool = bsdf.num_components(
                                                            bsdf_flags,
                                                            &arena_bxdf,
                                                        ) > 0;
                                                        if is_diffuse
                ...
                std::mem::drop(grid);
                {
                        ...
                        (self.photons_per_iteration / num_cores as i32) as usize;
                    {
						...
						isect.compute_scattering_functions(&photon_ray, &mut arena_bsdf, &mut arena_bxdf, true, mode);
                                                if let Some(ref photon_bsdf) = isect.get_bsdf(&arena_bsdf) {
