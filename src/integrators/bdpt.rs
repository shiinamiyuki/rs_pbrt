...
impl<'a> Vertex<'a> {
    ...
    pub fn f(
        ...
        match self.vertex_type {
            VertexType::Surface => {
                if let Some(ref si) = self.si {
                    if let Some(ref bsdf) = si.get_bsdf(arena_bsdf) {
                        let bsdf_flags: u8 = BxdfType::BsdfAll as u8;
                        bsdf.f(&si.common.wo, &wi, bsdf_flags, arena_bxdf)
    ...
    pub fn is_connectible(&self, arena_bsdf: &mut Vec<Bsdf>, arena_bxdf: &mut Vec<Bxdf>) -> bool {
        match self.vertex_type {
            ...
            VertexType::Surface => {
                if let Some(ref si) = self.si {
                    if let Some(ref bsdf) = si.get_bsdf(arena_bsdf) {
                        ...
                        bsdf.num_components(bsdf_flags, arena_bxdf) > 0
                    } else {
                        false
                    }
                } else {
    ...
    pub fn pdf(
        ...
        if self.vertex_type == VertexType::Camera {
            ...
            if let Some(ref si) = self.si {
                if let Some(ref bsdf) = si.get_bsdf(arena_bsdf) {
                    let bsdf_flags: u8 = BxdfType::BsdfAll as u8;
                    pdf_flt = bsdf.pdf(&wp, &wn, bsdf_flags, arena_bxdf);
                }
            }
        ...
        if let Some(mi) = mi_opt {
            ...
            if let Some(ref bsdf) = isect.get_bsdf(arena_bsdf) {
                ...
                *beta *= f * vec3_abs_dot_nrmf(&wi, &isect_shading_n) / pdf_fwd;
                ...
                pdf_rev = bsdf.pdf(&wi, &isect_wo, bsdf_flags, arena_bxdf);
    ...
    if let Some(ref mut overwrite) = pt {
        if s > 0 {
            if let Some(ref callable) = qs {
                if s > 1 {
                    overwrite.pdf_rev = callable.pdf(
                        scene,
                    ...
                    overwrite.pdf_rev =
                        callable.pdf(scene, None, &overwrite, arena_bsdf, arena_bxdf);
                }
            }
        } else if t > 1 {
            overwrite.pdf_rev =
        ...
        if t > 1 {
            ...
            if s > 0 {
                if let Some(ref qs_ref) = qs {
                    ...
                    pdf_rev =
                        callable.pdf(scene, None, &camera_vertices[t - 2], arena_bsdf, arena_bxdf);
                }
            } else {
    ...
    if let Some(ref mut overwrite) = qs {
        if let Some(ref callable) = pt {
            if let Some(ref pt_ref) = pt_minus {
                overwrite.pdf_rev =
                    callable.pdf(scene, Some(&pt_ref), &overwrite, arena_bsdf, arena_bxdf);
            } else {
                overwrite.pdf_rev = callable.pdf(scene, None, &overwrite, arena_bsdf, arena_bxdf);
            }
        }
    }
    if let Some(ref callable) = qs {
        if s > 1 {
            ...
            if let Some(ref pt_ref) = pt {
                ...
                pdf_rev = callable.pdf(scene, None, &light_vertices[s - 2], arena_bsdf, arena_bxdf);
            }
...
pub fn connect_bdpt<'a>(
    ...
    if s == 0 {
        ...
        if light_vertices[s - 1].is_connectible(arena_bsdf, arena_bxdf) {
            let mut iref: InteractionCommon = InteractionCommon::default();
        ...
        if camera_vertices[t - 1].is_connectible(arena_bsdf, arena_bxdf) {
        ...
        if light_vertices[s - 1].is_connectible(arena_bsdf, arena_bxdf)
            && camera_vertices[t - 1].is_connectible(arena_bsdf, arena_bxdf)
        {
            let radiance = camera_vertices[t - 1].f(
                &light_vertices[s - 1],
                TransportMode::Radiance,
                arena_bsdf,
                arena_bxdf,
            );
            let importance = light_vertices[s - 1].f(
                &camera_vertices[t - 1],
                TransportMode::Importance,
                arena_bsdf,
                arena_bxdf,
